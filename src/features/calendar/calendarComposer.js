...
import { createCalendarController } from './calendarController.js';
...
function loadWeekStartDay(storage = localStorage) {
  try {
    const saved = Number(storage.getItem(WEEK_START_STORAGE_KEY));
...
export function createCalendarComposer({ host = window, root = document } = {}) {
  // 모달 관련 DOM
  const taskModal = root.getElementById('taskModal');
  ...
  const engine = createCalendarEngine({
    initialState: {
      tasks: host.customTasks,
      taskStatus: host.taskStatus,
      viewMode: host.__calendarViewMode,
      weekStartDay: loadWeekStartDay(host.localStorage)
    }
  });
  const controller = createCalendarController({
    engine,
    ensureLogin: () => !!host.ensureLogin?.(),
    host,
    render: () => renderAll(),
    saveNow: () => host.cloudSaveAll?.(),
    saveSettings: () => host.cloudSaveCalendarSettings?.()
  });
  const getCalendarState = () => controller.getSnapshot();
  const getCalendarViewMode = () => getCalendarState().viewMode;
  ...
  const getRelativeWeekTitle = (weekStart) => {
    ...
    const todayWeekStart = startOfWeek(toKST(new Date()), getCalendarState().weekStartDay);
  ...
  const changeWeek = async (offset) => {
    ...
    controller.moveWeek(offset, { shouldRender: false });
    renderCalendar();
    ...
    weekTasks.forEach((task) => {
      ...
      if (weekOffset === 0) {
        taskElement.addEventListener('click', async (event) => {
          ...
          if (event.detail === 1) {
            await controller.toggleTaskComplete(task.id, isRecurringTask(task) ? fullDate : '');
  ...
  const createWeekRow = (weekOffset) => {
    ...
    const weekStart = addDays(calendarState.weekStart, weekOffset * 7);
    const weekStartYmd = ymdKST(weekStart);
    const weekEndYmd = ymdKST(addDays(weekStart, 6));
    const weekTasks = calendarState.tasks
      .filter((task) => {
    ...
    if (ymdKST(weekStart) === ymdKST(startOfWeek(toKST(new Date()), calendarState.weekStartDay))) {
  ...
  const renderMonthCalendar = (calendarGrid) => {
    ...
    for (let index = 0; index < 7; index += 1) {
      const weekday = document.createElement('span');
      const weekdayIndex = (calendarState.weekStartDay + index) % 7;
    ...
    undatedTaskList.replaceChildren(
      ...tasks.map((task) => {
        ...
        completeButton.addEventListener('click', async () => {
          await controller.toggleTaskComplete(task.id);
        });
  ...
  const renderAgendaList = () => {
    ...
    const tasks = getCalendarState().tasks;
    ...
    const makeRow = (t, { showDate }) => {
      ...
      row.addEventListener('click', async () => {
        await controller.toggleTaskComplete(t.id);
      });
      return row;
    };
  ...
  const renderTaskMiniCalendar = () => {
    ...
    const firstDate = new Date(year, month, 1);
    const weekStartDay = getCalendarState().weekStartDay;
  ...
  const openModal = (task = null) => {
    host.currentTask = task?.id ? controller.findTask(task.id) : task;
  ...
  const openCalendarSettings = () => {
    ...
    const calendarState = getCalendarState();
    calendarViewModeSelect.value = calendarState.viewMode;
    calendarWeekStartSelect.value = String(calendarState.weekStartDay);
  ...
  const saveCalendarSettings = () => {
    ...
    try {
      host.localStorage.setItem(WEEK_START_STORAGE_KEY, String(nextWeekStartDay));
    } catch {
      // 저장이 제한된 환경에서도 현재 세션에는 설정을 적용한다.
    }
    closeCalendarSettings();
    controller.saveCalendarSettings({ viewMode: nextViewMode, weekStartDay: nextWeekStartDay });
  };

  const saveTask = async () => {
    if (!host.ensureLogin?.()) return;
    ...
    if (repeatEnabled && repeatDays.length === 0) {
      host.showFeedbackMessage?.('반복할 요일을 선택해주세요.');
      return;
    }
    const countEnabled = repeatEnabled && !!taskCountEnabledInput?.checked;
    const countAnchor =
    ...
    if (!title && !repeatEnabled) {
      host.showFeedbackMessage?.('제목을 입력해주세요.');
      return;
    }
    const data = {
      id: host.currentTask?.id || Date.now(),
      title,
      description,
      date,
      endDate,
      ...
      occurrenceStatus: host.currentTask?.occurrenceStatus || {},
      todoOnly: !date,
      complete: host.currentTask?.complete ?? false
    };
    await controller.upsertTask(data);
    closeModal();
  };

  const deleteCurrentTask = async () => {
    if (!host.currentTask?.id) {
      closeModal();
      return false;
    }
    const deleted = await controller.deleteTask(host.currentTask.id);
  ...
  host.__calendarControllerCompatibility = controller;
  host.renderCalendar = controller.render;
  host.openCalendarSettings = openCalendarSettings;
  host.openTaskModal = openModal;
  host.closeTaskModal = closeModal;
  host.deleteTask = deleteCurrentTask;
  controller.publish();
  renderAll();
  return {
    controller,
    engine,
    renderFromCompatibility() {
      controller.hydrate({
        tasks: host.customTasks,
        taskStatus: host.taskStatus,
        viewMode: host.__calendarViewMode,
        weekStartDay: getCalendarState().weekStartDay,
        weekAnchor: getCalendarState().weekStart,
        monthAnchor: getCalendarState().monthDate
      });
    }
  };
}
