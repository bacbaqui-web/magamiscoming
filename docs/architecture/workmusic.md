...
- 삼각형 pointerdown은 기존 음파 뷰포트 안에서 SVG viewBox를 280ms에 걸쳐 확대한다. 별도 상세 행이나 페이지 높이 변화는 없다.
- 포인터 release/cancel/창 blur 시 원래 음파로 자동 복귀한다. 전체 보기/Escape도 유지한다.
- 원래 핸들 드래그도 상세 축을 사용한다. 시각적 보간은 기존 RMS 요약을 연결할 뿐 실제 샘플 해상도를 늘리지 않는다.
