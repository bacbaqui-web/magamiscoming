...
- 새 접속/저장 데이터 적용 후 모드는 랜덤이다. 순서 전환은 현재 Player를 다시 만들지 않고 다음 후보를 갱신한다. 목록 데이터가 같으면 카드를 재사용하며 순서만 변경한다.
- 메인은 활성/대기 Player 구성을 유지하며 DJ 전환 때 재생 중 Player를 다시 만들지 않는다. DJ 꺼짐은 곡 끝에서 겹침 없이 전환한다. 모드 전환 UI는 저장 응답을 기다리지 않는다.
- 현재 곡 삭제 시 저장 목록에서는 제거하고 Engine 세션의 playbackOnly 곡으로 재생을 유지한다. 목록에는 표시하지 않고 다음 곡 선택/탭 변경 때 정리한다. 다른 곡 삭제도 현재 곡 ID와 재생 순서를 보존한다.

- 자동 초록 시작이 10초 미만이면 10초 이상이고 초록 종료보다 앞인 가장 이른 분석 구간 끝으로 이동한다. 유효 경계가 없으면 기존 분석 위치를 유지하고 임의의 10초 지점에서 자르지 않는다. 수동값은 보정하지 않는다.
- 메인/리모컨 음량 input은 음량 UI만 갱신하며 저장은 change에서 수행한다. 목록 전체 렌더를 호출하지 않는다.

- 자동 분석 상태 오른쪽 재분석 아이콘은 현재 곡을 기존 단일 분석 큐에 요청한다. 진행 중 중복 요청은 막고 수동 구간을 보존한다.
...
- 자동 분석 원본은 저장하지 않는다. 사용자가 수정한 `mediaAnalysisManual`의
  ...
  이전 두 필드 데이터도 그대로 읽는다. 자동 1절은 초록 시작에서 60초 이상 지난 후렴/간주 끝 중 가장 이른 경계를 선택하며 초록 종료를 넘지 않는다. 유효 후보가 없으면 초록 종료를 사용한다. 수동 1절은 유지한다.
