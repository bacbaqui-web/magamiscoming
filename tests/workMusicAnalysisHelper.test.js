...
test('DJ previews the next intro for up to ten seconds and mutes the previous song at green', () => {
  for (const outro of [0, 0.15, 1, 4, 12, 30]) {
    for (const intro of [0, 0.25, 3, 15, 40]) {
      for (const late of [0, 0.5, 10]) {
        ...
        assert.equal(plan.outroSeconds, 0);
...
test('DJ uses displayed cached range, but saved edits take priority; verse is not a playback cutoff', () => {
  ...
  assert.equal(calculateDjTransitionPlan(input).outroSeconds, 0);
  assert.equal(calculateDjTransitionPlan({ ...input, verseMode: true }).outroSeconds, 0);
